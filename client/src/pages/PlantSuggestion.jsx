...
// Stable component so chat state is not reset when quiz state (currentStep/answers) changes
const PlantSuggestionChatPanel = ({ user, onAddToGarden }) => (
  <div
    className="relative min-h-screen lg:min-h-full w-full flex flex-col items-center justify-center py-8 px-4"
    style={{
      backgroundImage: `linear-gradient(180deg, rgba(2, 44, 34, 0.94) 0%, rgba(6, 78, 59, 0.9) 50%, rgba(4, 47, 46, 0.96) 100%),
  ...
  >
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      <div className="absolute top-[5%] left-[5%] text-6xl lg:text-7xl opacity-20">🍎</div>
      <div className="absolute top-[10%] right-[8%] text-5xl lg:text-6xl opacity-20">🍐</div>
      <div className="absolute bottom-[15%] left-[7%] text-5xl lg:text-6xl opacity-20">🍋</div>
      <div className="absolute bottom-[10%] right-[5%] text-6xl lg:text-7xl opacity-20">🍇</div>
      <div className="absolute top-[40%] left-[3%] text-4xl lg:text-5xl opacity-15">🥝</div>
      <div className="absolute top-[35%] right-[4%] text-4xl lg:text-5xl opacity-15">🍊</div>
      <div className="absolute bottom-[40%] right-[12%] text-4xl lg:text-5xl opacity-15">🍓</div>
      <div className="absolute bottom-[35%] left-[10%] text-4xl lg:text-5xl opacity-15">🍑</div>
    </div>
    <div className="relative z-10 w-full max-w-xl mx-auto flex flex-col flex-1 min-h-0">
      <AnimatedAIChat user={user} onAddToGarden={onAddToGarden} />
    </div>
  </div>
);
...
const PlantSuggestion = () => {
  ...
  const handleAddToGarden = async (plant) => {
    const saveLocally = () => {
      try {
        ...
        const imagesKey = `plant_images_${storageKeyBase}`;
    ...
    // For guests or when offline, never call the API – just save locally
    ...
    try {
      ...
      alert(
        'Could not save this plant to your online garden. ' +
        'Please try again later. Your My Garden Journal will only show plants saved successfully to the server.'
      );
    }
  };
  ...
  return (
    <div className="min-h-screen overflow-x-hidden flex flex-col lg:flex-row">
      {/* Left half: Plant suggestion (quiz / results / loading) */}
      <section className="min-h-[50vh] lg:min-h-screen lg:w-1/2 bg-gradient-to-br from-forest-green-50 via-cream-100 to-forest-green-100 py-6 lg:py-8 relative overflow-auto">
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        {isLoading && (
          <div className="min-h-[50vh] flex flex-col items-center justify-center relative z-10 px-4">
        ...
        {!isLoading && results && (
          <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-6 relative z-10">
            ...
            {results.plants.length > 0 ? (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="grid grid-cols-1 gap-4 mb-6 max-h-[60vh] overflow-y-auto pr-2">
                {results.plants.map((plant, index) => (
                  <motion.div key={plant.name} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + index * 0.1 }} className="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow border border-white/20 flex flex-row">
                    <div className="relative w-32 shrink-0 h-28">
                    ...
                    <div className="p-4 flex-1 flex flex-col justify-between min-w-0">
                      <div>
                        ...
                        <p className="text-gray-600 text-sm line-clamp-2 mb-2">{plant.description}</p>
                        <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-gray-500">
                          <span>{plant.growingTime}</span>
                          <span>{plant.sunlight}</span>
                          <span>{plant.difficulty}</span>
                        </div>
                      </div>
                      <button onClick={() => handleAddToGarden(plant)} className="mt-2 w-full bg-forest-green-600 text-white py-2 px-3 rounded-lg hover:bg-forest-green-700 transition-colors text-sm">
                        Add to My Garden
                      </button>
                    </div>
                  </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-center py-12">
                <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-8 max-w-md mx-auto border border-white/20">
                  ...
                  <div className="space-y-3">
                    ...
                    {results.combinationKey === 'empty' && (
                      <p className="text-sm text-gray-500">💡 Tip: Use Postman to add plants to your database first!</p>
                    )}
                  </div>
                </div>
              </motion.div>
            ...
            <div className="text-center">
              <button onClick={resetQuiz} className="bg-gray-600 text-white px-5 py-2.5 rounded-lg hover:bg-gray-700 transition-colors mr-3 text-sm">
                Take Quiz Again
              </button>
              {results.plants.length > 0 && (
                <button onClick={() => (window.location.href = '/my-garden-journal')} className="bg-forest-green-600 text-white px-5 py-2.5 rounded-lg hover:bg-forest-green-700 transition-colors text-sm">
        ...
        {!isLoading && !results && (
          <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-6 relative z-10">
            ...
            <div className="flex justify-between">
              <button onClick={prevStep} disabled={currentStep === 0} className={`flex items-center px-5 py-2.5 rounded-lg transition-colors text-sm ${currentStep === 0 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-gray-600 text-white hover:bg-gray-700'}`}>
                <FaArrowLeft className="mr-2" />
                Previous
              </button>
              <button onClick={nextStep} disabled={!answers[currentQuestion.id]} className={`flex items-center px-5 py-2.5 rounded-lg transition-colors text-sm ${!answers[currentQuestion.id] ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-forest-green-600 text-white hover:bg-forest-green-700'}`}>
      ...
      {/* Right half: Chat assistant with fruits/veg background — stable component so chat doesn't reset on quiz step change */}
      <aside className="min-h-[50vh] lg:min-h-screen lg:w-1/2 flex flex-col">
        <PlantSuggestionChatPanel user={user} onAddToGarden={handleAddToGarden} />
      </aside>
    </div>
  );
};
